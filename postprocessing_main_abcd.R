...
fx$fig_4$a <- ggplot(subset(data4, mask_col & vd != 0 & modality == "area"), aes(x = vd, group = interaction(model, vs), color = model, linetype = vs)) + 
...
fx$fig_4$c <- ggplot(subset(data4, mask_col & vd != 0 & modality == "volume"), aes(x = vd, group = interaction(model, vs), color = model, linetype = vs)) + 
  geom_density(n = 500, size = 1.2) + 
  theme_classic() + 
  ylab("Density") + 
  xlab("Coefficient") +
...
# file names + paths for overlay plots we want
overl_name <- expand.grid(hemis, "_", project_date, ".",c("area", "volume"), "_special_model_3_overlay_to_model5.mgh" ) %>%
  apply(1, paste, collapse = "")
overl_paths <- file.path(dir_out, overl_name)
...
fx$fig_6$row3 <- magick::image_append(c(ofig3, ofig4))
...
dfig2 <- quick_compose(delta_paths[2], hemis[2], head = TRUE, head_size = head_size, cs = TRUE, cs_size = cs_size, cs_value = cs_value, cs_offset = c("+5+18", "+8+238"))
...
dfig4 <- quick_compose(delta_paths[4], hemis[2], head = TRUE, head_size = head_size, cs = TRUE, cs_size = cs_size, cs_value = cs_value, cs_offset = c("+5+18", "+8+238"))

fx$fig_s2 <- list()
fx$fig_s2$row1 <- magick::image_append(c(dfig1, dfig2))
fx$fig_s2$row3 <- magick::image_append(c(dfig3, dfig4))
