...
#### PRE INIT
main_dir <- "PATH/TO/YOUR/ABCD/WORKING/DIRECTORY"

#### FUNCTIONS
source("postprocessing_functions.R")

#### STUDY PARAMETERS
source("postprocessing_parameters_abcd.R")
...
## Figure 1 / Supplementary Figure 2
    ...
    ofig1 <- quick_compose(overl_paths[1], hemis[1], cs = FALSE, text = "ABCD", text_size = text_size, head = TRUE, head_size = head_size, alt_text = "Area")
    ofig2 <- quick_compose(overl_paths[2], hemis[2], cs = FALSE, head = TRUE, head_size = head_size)
    ofig3 <- quick_compose(overl_paths[3], hemis[1], cs = FALSE, text = "ABCD", text_size = text_size, head = TRUE, head_size = head_size, alt_text = "Volume")
    ofig4 <- quick_compose(overl_paths[4], hemis[2], cs = FALSE, head = TRUE, head_size = head_size)
    ofig5 <- quick_compose(overl_paths[5], hemis[1], cs = FALSE, text = "ABCD", text_size = text_size, head = TRUE, head_size = head_size, alt_text = "Thickness")
    ofig6 <- quick_compose(overl_paths[6], hemis[2], cs = FALSE, head = TRUE, head_size = head_size)
    
    fx$fig_1 <- list()
    fx$fig_1$row1 <- magick::image_append(c(ofig1, ofig2))
    
    fx$sfig_2 <- list()
    fx$sfig_2$row1 <- magick::image_append(c(ofig3, ofig4))
    fx$sfig_2$row3 <- magick::image_append(c(ofig5, ofig6))
...
## Figure 3 / Supplementary Figure 2
    ...
    fx$sfig_3 <- list()
    fx$sfig_3$row1 <- magick::image_append(c(ofig3, ofig4))
    fx$sfig_3$row3 <- magick::image_append(c(ofig5, ofig6))
    
## FIGURE 4 (outcome -> hemisphere -> model)

  fx$fig_4a <- ggplot2::ggplot(subset(roi_list[[1]][[2]], names != "unknown"),
...
## Figure 6 / supplement
    ...
    fx$fig_6 <- list()
    fx$fig_6$row1 <- magick::image_append(c(ofig1, ofig2))
    
    fx$sfig_11 <- list()
    fx$sfig_11$row1 <- magick::image_append(c(ofig3, ofig4))
    fx$sfig_11$row3 <- magick::image_append(c(ofig5, ofig6))
...
## Figure 8 / Supplementary Figure 12
    ...
    fx$fig_8 <- list()
    fx$fig_8$row1 <- magick::image_append(c(ofig1, ofig2))
    
    fx$sfig_12 <- list()
    fx$sfig_12$row1 <- magick::image_append(c(ofig3, ofig4))
    fx$sfig_12$row3 <- magick::image_append(c(ofig5, ofig6))
...
## Supplementary Figure 6
  ...
  # create composite images for left and right, then combine
  dfig1 <- quick_compose(delta_paths[1], hemis[1], cs = FALSE, text = "ABCD", text_size = text_size, head = TRUE, head_size = head_size, alt_text = "Area")
  dfig2 <- quick_compose(delta_paths[2], hemis[2], head = TRUE, head_size = head_size, cs = TRUE, cs_size = cs_size, cs_value = cs_value, cs_offset = c("+5+0", "+8+218"))
  dfig3 <- quick_compose(delta_paths[3], hemis[1], cs = FALSE, text = "ABCD", text_size = text_size, head = TRUE, head_size = head_size, alt_text = "Volume")
  dfig4 <- quick_compose(delta_paths[4], hemis[2], head = TRUE, head_size = head_size, cs = TRUE, cs_size = cs_size, cs_value = cs_value, cs_offset = c("+5+0", "+8+218"))
  dfig5 <- quick_compose(delta_paths[5], hemis[1], cs = FALSE, text = "ABCD", text_size = text_size, head = TRUE, head_size = head_size, alt_text = "Thickness")
  dfig6 <- quick_compose(delta_paths[6], hemis[2], head = TRUE, head_size = head_size, cs = TRUE, cs_size = cs_size, cs_value = cs_value, cs_offset = c("+5+0", "+8+218"))
  
  fx$sfig_6 <- list()
  fx$sfig_6$row1 <- magick::image_append(c(dfig1, dfig2))
  fx$sfig_6$row3 <- magick::image_append(c(dfig3, dfig4))
  fx$sfig_6$row5 <- magick::image_append(c(dfig5, dfig6))

## Supplementary Figure 7

  # scatterplot model 1 vs % change from 1 to 2
  fx$sfig_7a <- ggplot2::ggplot(data = subset(roi_list[[1]][[1]], names != "unknown"), 
  ...
  fx$sfig_7b <- ggplot2::ggplot(data = subset(roi_list[[1]][[1]], names != "unknown"), 
...
## Supplementary Figure 8
  ...
  fx$sfig_8a <- ggplot2::ggplot(subset(roi_list[[2]][[2]], names != "unknown"),
               ggplot2::aes(x = model, y = names, group = time, color = time)) +
  ggplot2::geom_point() +
  ggplot2::geom_hline(yintercept = 1:100, linetype = "dotted", color = "gray90") +
  ggplot2::geom_vline(xintercept = 0, color = "grey") +
...
## Supplementary Figure 9
  ...
  # scatterplot model 1 vs % change from 1 to 2
  fx$sfig_9a <- ggplot2::ggplot(data = subset(roi_list[[2]][[1]], names != "unknown"), 
  			  ggplot2::aes(x = model_1, y = deltaperc12)) + 
  ggplot2::geom_point() + 
  ggplot2::geom_hline(yintercept = 0, linetype = "dotted") + 
  #ggrepel::geom_text_repel(ggplot2::aes(label = names), size = 4, max.overlaps = 20, box.padding = 1, segment.linetype = 3) + 
  ...
  # scatterplot model 1 vs % change from 2 to 3
  fx$sfig_9b <- ggplot2::ggplot(data = subset(roi_list[[2]][[1]], names != "unknown"), 
  			  ggplot2::aes(x = model_2, y = deltaperc23)) + 
  ggplot2::geom_point() + 
  ggplot2::geom_hline(yintercept = 0, linetype = "dotted") + 
  #ggrepel::geom_text_repel(ggplot2::aes(label = names), size = 4, max.overlaps = 20, box.padding = 1, segment.linetype = 3) + 
...
out_path <- file.path(main_dir, "abcd.rds")
